...
bc_dm = DistanceMatrix(bc_dist, ids=muni_inf_filt['sample_id'].tolist())

# PCoA
pcoa_result = pcoa(bc_dm)
pcoa_df = pcoa_result.samples.copy()
pcoa_df['country'] = muni_inf_filt['country_std'].values

print(f"\nPCoA - 前3轴解释方差: {pcoa_result.proportion_explained[:3].values}")

# 国家间差异显著性 - PERMANOVA（手动简化版）
print("\n国家间ARG组成差异检验 (简化PERMANOVA):")

# 计算国家间平均距离 vs 国家内平均距离
between_country_dists = []
within_country_dists = []

sample_country_map = dict(zip(muni_inf_filt['sample_id'], muni_inf_filt['country_std']))

for i, sid1 in enumerate(muni_inf_filt['sample_id']):
    for j, sid2 in enumerate(muni_inf_filt['sample_id']):
        if i < j:
            dist = bc_dist[i, j]
            if sample_country_map[sid1] == sample_country_map[sid2]:
...
print(f"  国家间平均距离: {mean_between:.3f}")
print(f"  国家内平均距离: {mean_within:.3f}")
...
bc_microbiome_dm = DistanceMatrix(bc_microbiome, ids=merged_bracken['sample_id'].tolist())

# PCoA
pcoa_microbiome = pcoa(bc_microbiome_dm)
print(f"\n微生物群落PCoA - 前3轴解释方差: {pcoa_microbiome.proportion_explained[:3].values}")

# 国家间微生物群落差异
between_micro = []
within_micro = []

sample_country_map_micro = dict(zip(merged_bracken['sample_id'], merged_bracken['country_std']))

for i, sid1 in enumerate(merged_bracken['sample_id']):
    for j, sid2 in enumerate(merged_bracken['sample_id']):
        if i < j:
            dist = bc_microbiome[i, j]
            if sample_country_map_micro[sid1] == sample_country_map_micro[sid2]:
...
print(f"\n微生物群落国家间平均距离: {mean_between_micro:.3f}")
print(f"微生物群落国家内平均距离: {mean_within_micro:.3f}")
...
print(f"Mann-Whitney U test: p = {p_val_micro:.4f}")
if p_val_micro < 0.05:
...
plasmid_groups = [mge_stats_df[mge_stats_df['country']==c]['plasmid_pct'].values for c in countries_enough]
h_plasmid, p_plasmid = stats.kruskal(*plasmid_groups)
print(f"  质粒携带率: H = {h_plasmid:.2f}, p = {p_plasmid:.4f}")

virus_groups = [mge_stats_df[mge_stats_df['country']==c]['virus_pct'].values for c in countries_enough]
h_virus, p_virus = stats.kruskal(*virus_groups)
print(f"  噬菌体携带率: H = {h_virus:.2f}, p = {p_virus:.4f}")
...
for country in countries_enough:
    ...
    if len(country_data) >= 3:
        ...
        time_span = f"{years.min():.0f}-{years.max():.0f}"
